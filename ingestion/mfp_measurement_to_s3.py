...
PREFIX = "mfp/measurements"


def get_latest_file():
    files = list(DATA_FOLDER.glob("*Measurement-Summary*.csv"))
    if not files:
        raise FileNotFoundError(f"No measurement CSV files found in {DATA_FOLDER.resolve()}")
